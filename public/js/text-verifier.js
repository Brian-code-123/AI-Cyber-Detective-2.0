...
function renderTextResults(data) {
  ...
  if (cred >= 75) {
    color = "#00ff41";
    label = "✅ " + t("text.likelyCredible");
  } else if (cred >= 50) {
    color = "#ffaa00";
    label = "⚠️ " + t("text.questionable");
  } else if (cred >= 25) {
    color = "#ff6600";
    label = "🔶 " + t("text.suspicious");
  } else {
    color = "#ff0055";
    label = "🚨 " + t("text.likelyMisinformation");
  }
  ...
  const statClaims = (
    rawText.match(/\d+%|\d+ out of \d+|\d+ million|\d+ billion/g) || []
  ).length;
  const hasCitations =
  ...
  ct.innerHTML = Object.entries({
    ...
    [t("text.avgWords")]: sentences.length
    ...
    [t("text.sourceCitations")]: hasCitations
      ? t("text.yes")
      : t("text.noneFound"),
  })
  ...
  if (sc > 0.05) {
    sentColor = "#00ff41";
    sentEmoji = "😊";
    sentLabel = t("text.positive");
  } else if (sc < -0.05) {
    sentColor = "#ff0055";
    sentEmoji = "😠";
    sentLabel = t("text.negative");
  } else {
    sentColor = "#ffaa00";
    sentEmoji = "😐";
    sentLabel = t("text.neutral");
  }
  sentViz.innerHTML = `
  ...
  if (allFindings.length === 0) {
    ...
    allFindings.forEach((f) => {
      ...
      const sevClass =
        type === "warning" ? "warning" : type === "safe" ? "safe" : "info";
