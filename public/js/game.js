...
async function prefetchAIScenarios() {
  ...
  const lang = (typeof currentLang !== 'undefined' ? currentLang : localStorage.getItem('cyberlang')) || 'en';
  try {
    const res = await fetch(`/api/game-scenario?tier=${gameState.currentTier}&lang=${lang}`);
    if (res.ok) {
  ...
  } catch(e) { /* silently fail — hardcoded scenarios always available */ }
  ...
  try {
    const res = await fetch('/api/chatbot', {
      ...
      body: JSON.stringify({
        ...
        history: [],
        context: 'Training Game mode — the user answered incorrectly and needs a brief educational explanation.'
      })
    });
    if (res.ok) {
      const data = await res.json();
      if (data.reply) {
        ...
        aiDiv.style.cssText = 'margin-top:12px;padding:10px 14px;background:rgba(10,132,255,0.06);border-radius:10px;border-left:3px solid var(--accent);font-size:0.88rem;color:var(--text-secondary);';
        aiDiv.innerHTML = '🤖 <strong style="color:var(--accent)">AI Tip:</strong> ' + data.reply;
