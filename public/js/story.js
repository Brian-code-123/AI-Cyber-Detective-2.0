...
async function askStoryAI(chapterNum) {
  ...
  if (!userQ) {
    replyDiv.innerHTML = '<span style="color:var(--orange)">Please type a question about this chapter first.</span>';
    replyDiv.style.display = 'block';
    return;
  }

  // Show loading
  if (btn) { btn.disabled = true; btn.innerHTML = '<span class="typing-dots"><span></span><span></span><span></span></span>'; }
  replyDiv.style.display = 'block';
  ...
  try {
    ...
    replyDiv.innerHTML = '🤖 <strong>NeoTrace AI:</strong> ' + reply.replace(/\n/g, '<br>');
  } catch(err) {
    replyDiv.innerHTML = '<span style="color:var(--orange)">⚡ AI temporarily unavailable. The chapter\'s Detective Analysis above has the key takeaways.</span>';
  } finally {
    if (btn) {
      btn.disabled = false;
      const lang = localStorage.getItem('cyberlang') || 'en';
      btn.innerHTML = '<span data-i18n="story.askBtn">' + (lang === 'zh' ? '問AI' : 'Ask AI') + '</span> ➤';
    }
  }
}
